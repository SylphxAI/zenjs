...
import { signal, computed, batch, effect } from 'zenjs';
  ...
  return (
    <div class="demo-card" style="border: 2px solid #00d4ff;">
      ...
      <div style="background: #f0f9ff; padding: 16px; border-radius: 8px; margin: 12px 0;">
        ...
        <p style="margin: 8px 0;"><strong>只更新改變的值</strong> - 不re-render整個component</p>
        <p style="margin: 8px 0;"><strong>Component 只執行一次</strong> - 之後全靠 Signal 自動更新</p>
        <p style="margin: 8px 0;"><strong>零 Virtual DOM</strong> - 直接操作真實 DOM</p>
      </div>
      ...
      <div style="background: #e7f5ff; padding: 12px; border-radius: 6px; margin-top: 16px;">
        <strong>總點擊次數:</strong> <span style="font-size: 24px; color: #00d4ff;">{totalClicks}</span>
      ...
      <div style="background: #fff3cd; padding: 12px; border-radius: 6px; margin-top: 12px;">
        ...
        <p style="margin: 8px 0 0 0; font-size: 14px;">
          React 需要 re-render 整個 component → 創建新的 virtual DOM → diff → 更新真實 DOM<br/>
  ...
  return (
    <div class="demo-card" style="border: 2px solid #ff6b6b;">
      ...
      <div style="background: #fff5f5; padding: 16px; border-radius: 8px; margin: 12px 0;">
        ...
        <p style="margin: 8px 0;"><strong>Microtask Batching</strong> - 自動合併更新到下一個 microtask</p>
        <p style="margin: 8px 0;"><strong>減少重複計算</strong> - 多個改變只觸發一次更新</p>
        <p style="margin: 8px 0;"><strong>Smart Scheduling</strong> - 使用 queueMicrotask 優化性能</p>
      </div>
      ...
      <div style="background: #fff3cd; padding: 12px; border-radius: 6px; margin-top: 12px;">
        ...
        <p style="margin: 8px 0; font-size: 14px;">
          不用 batch: 每次 count.value++ 都觸發 sum 重新計算<br/>
          用 batch: 3次改變合併，sum 只計算一次 → <strong>省 66% 運算</strong>
        </p>
      </div>
    </div>
  );
    ...
    <div class="demo-card" style="border: 2px solid #845ef7;">
      ...
      <div style="background: #f3f0ff; padding: 16px; border-radius: 8px;">
        ...
        <div style="background: #e5dbff; padding: 12px; border-radius: 6px;">
          <strong>📊 綜合效果:</strong> 相比標準實現，內存使用減少約 <strong style="color: #5f3dc4;">40-60%</strong>
        </div>
      </div>
    </div>
  );
